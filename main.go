...
var (
	...
	generatedPaths []string // 记录所有生成的路径
...
func loadConfig() error {
	...
	if err != nil {
		return fmt.Errorf("获取家目录失败: %v", err)
	}

	// 读取config.yaml文件内容
	data, err := os.ReadFile(configPath)
	if err != nil {
		return fmt.Errorf("读取配置文件失败(已尝试 %s): %v", configPath, err)
	}
	...
	if err != nil {
		// 打印原始配置文件内容用于调试
		//fmt.Printf("配置文件原始内容:\n%s\n", string(data))
		return fmt.Errorf("解析配置文件失败: %v (文件路径: %s)", err, configPath)
	}

	// 替换路径中的特殊关键字
	replaceKeywords := func(path *string) {
		if path != nil {
			*path = strings.ReplaceAll(*path, "[user]", currentUser.Username)
			*path = strings.ReplaceAll(*path, "[home]", homeDir)
		}
	}

	// 处理所有路径配置
	replaceKeywords(&Config.AlacSaveFolder)
	replaceKeywords(&Config.AtmosSaveFolder)
	replaceKeywords(&Config.ArtistFolderFormat)
	replaceKeywords(&Config.AlbumFolderFormat)
	replaceKeywords(&Config.PlaylistFolderFormat)
...
func checkArtist(artistUrl string, token string, relationship string) ([]string, error) {
	...
	fmt.Println("请从上述" + relationship + "选项中选择(多个选项用逗号分隔,支持范围选择,或输入'all'选择全部)")
	cyanColor := color.New(color.FgCyan)
	cyanColor.Print("输入你的选择: ")
	...
	for _, opt := range selectedOptions {
		if len(opt) == 1 { // Single option
			...
			if err1 != nil || err2 != nil {
				fmt.Println("无效的范围:", opt)
				continue
			}
			if start < 1 || end > len(options) || start > end {
				fmt.Println("提供的范围超出了有效范围:", opt)
				continue
			}
			for i := start; i <= end; i++ {
...
func downloadTrack(trackNum int, trackTotal int, meta *structs.AutoGenerated, track structs.TrackData, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec string, covPath string) {
	counter.Total++
	fmt.Printf("第 %d 首,共 %d 首:\n", trackNum, trackTotal)

	//mv dl dev
	if track.Type == "music-videos" {
		if len(mediaUserToken) <= 50 {
			fmt.Println("meida-user-token 未设置或无效,无法下载MV")
			counter.Success++
			return
		}
		if _, err := exec.LookPath("mp4decrypt"); err != nil {
			fmt.Println("mp4decrypt 未找到, 无法下载MV")
			counter.Success++
			return
		}
		err := mvDownloader(track.ID, sanAlbumFolder, token, storefront, mediaUserToken, meta)
		if err != nil {
			fmt.Println("\u26A0 下载MV失败", err)
			counter.Error++
			return
		}
		counter.Success++
	...
	if manifest.Attributes.ExtendedAssetUrls.EnhancedHls == "" {
		if dl_atmos {
			fmt.Println("该音频质量不可用")
			counter.Unavailable++
			return
		}
		fmt.Println("该音频质量不可用,尝试下载AAC-LC")
		needDlAacLc = true
	}
	needCheck := false
...
func rip(albumId string, token string, storefront string, mediaUserToken string, urlArg_i string, isSingleSong bool) error {
		...
		for trackNum, track := range meta.Data[0].Relationships.Tracks.Data {
			...
			if needCheck {
				...
				if err == nil && strings.HasSuffix(fullM3u8Url, ".m3u8") {
					...
					fmt.Println("配置的的m3u8端口不工作,将使用来自 Web API 的 m3u8")
				}
			}

			_, _, err = extractMedia(m3u8Url, true)
			if err != nil {
				fmt.Printf("无法获取歌曲的音质信息 %d: %v\n", trackNum, err)
				continue
			}
		}
	...
	if Config.SaveAnimatedArtwork && meta.Data[0].Attributes.EditorialVideo.MotionDetailSquare.Video != "" {
		...
		if err != nil {
			fmt.Println("没有动态方形封面视频.\n", err)
		} else {
			exists, err := fileExists(filepath.Join(sanAlbumFolder, "square_animated_artwork.mp4"))
			if err != nil {
				fmt.Println("无法检查动态方形封面视频是否存在.")
			}
			if exists {
				fmt.Println("动态方形封面视频已存在.")
			} else {
				fmt.Println("下载动态方形封面视频中...")
				...
				if err := cmd.Run(); err != nil {
					fmt.Printf("动态方形封面视频下载失败: %v\n", err)
				} else {
					fmt.Println("已下载动态方形封面视频")
				}
			}
		}
		...
		if err != nil {
			...
			if err != nil {
				fmt.Println("无法检查竖版动态封面是否存在.")
			}
			if exists {
				fmt.Println("竖版动态封面已存在.")
			} else {
				fmt.Println("下载竖版动态封面中...")
				...
				if err := cmd.Run(); err != nil {
					fmt.Printf("竖版动态封面下载失败: %v\n", err)
				} else {
					fmt.Println("竖版动态封面已下载")
				}
			}
		}
	}
	...
	if !dl_select {
		...
		table := tablewriter.NewWriter(os.Stdout)
		table.SetHeader([]string{"", "曲目名称", "分级", "类型"})
		//table.SetFooter([]string{"", "", "Footer", "Footer4"})
		table.SetRowLine(false)
		//table.SetAutoMergeCells(true)
		table.SetCaption(meta.Data[0].Type == "albums", fmt.Sprintf("区域：%s,缺少 %d 首曲目", strings.ToUpper(storefront), meta.Data[0].Attributes.TrackCount-trackTotal))
		table.SetHeaderColor(tablewriter.Colors{},
			tablewriter.Colors{tablewriter.FgRedColor, tablewriter.Bold},
		...
		table.Render()
		fmt.Println("请从上述曲目选项中选择(多个选项用逗号分隔,支持范围选择,或输入'all'选择全部)")
		cyanColor := color.New(color.FgCyan)
		cyanColor.Print("请选择: ")
		reader := bufio.NewReader(os.Stdin)
...
func main() {
	...
	if err != nil {
		if Config.AuthorizationToken != "" && Config.AuthorizationToken != "your-authorization-token" {
			...
			fmt.Println("获取授权令牌失败")
			return
		}
	}
	// Define command-line flags
	pflag.BoolVar(&dl_atmos, "atmos", false, "启用atmos下载模式")
	pflag.BoolVar(&dl_aac, "aac", false, "启用adm-aac下载模式")
	pflag.BoolVar(&dl_select, "select", false, "启用选择下载模式(交互式选择专辑中的曲目)")
	pflag.BoolVar(&dl_song, "song", false, "启用单曲下载模式")
	pflag.BoolVar(&artist_select, "all-album", false, "下载艺术家的所有专辑和单曲")
	pflag.BoolVar(&debug_mode, "debug", false, "启用调试模式以展示音频质量信息(不下载)")
	alac_max = pflag.Int("alac-max", Config.AlacMax, "指定ALAC的最高音质")
	atmos_max = pflag.Int("atmos-max", Config.AtmosMax, "指定atmos的最高音质")
	aac_type = pflag.String("aac-type", Config.AacType, "指定AAC的类型:aac aac-binaural aac-downmix")
	mv_audio_type = pflag.String("mv-audio-type", Config.MVAudioType, "指定MV的音频类型:atmos ac3 aac")
	mv_max = pflag.Int("mv-max", Config.MVMax, "指定MV的最高品质")
	...
	pflag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "用法: %s [options] url1 url2 ...\n", "[amdl]")
		fmt.Fprintln(os.Stderr, "参数:")

		pflag.PrintDefaults()
		output := buf.String()
		output = strings.ReplaceAll(output, "(default ", "(默认值 ")
	...
	for {
		for albumNum, urlRaw := range os.Args {
			fmt.Printf("第 %d 个专辑,共 %d 个:\n", albumNum+1, albumTotal)
			var storefront, albumId string
			var isSingleSongTask bool
			//mv dl dev
			if strings.Contains(urlRaw, "/music-video/") {
				...
				if len(Config.MediaUserToken) <= 50 {
					fmt.Println("未设置media-user-token,跳过MV下载.")
					counter.Success++
					continue
				}
				if _, err := exec.LookPath("mp4decrypt"); err != nil {
					fmt.Println("mp4decrypt未找到,跳过MV下载.")
					counter.Success++
					continue
				}
			...
			if err != nil {
				log.Fatalf("URL无效: %v", err)
			}
			var urlArg_i = parse.Query().Get("i")
			err = rip(albumId, token, storefront, Config.MediaUserToken, urlArg_i, isSingleSongTask || dl_song)
			if err != nil {
				fmt.Println("专辑下载失败.")
				fmt.Println(err)
			}
		}
		fmt.Printf("=======  [\u2714 ] 完成: %d/%d  |  [\u26A0 ] 警告: %d  |  [\u2716 ] 错误: %d  =======\n", counter.Success, counter.Total, counter.Unavailable+counter.NotSong, counter.Error)
		if counter.Error == 0 {
			break
		}
		fmt.Println("检测到错误,按Enter重试")
		fmt.Scanln()
		fmt.Println("开始重试..")
		counter = structs.Counter{}
	}
	if len(generatedPaths) > 0 && counter.Success > 0 {
		for _, path := range generatedPaths {
			fmt.Printf("下载文件保存至:%s\n", path)
		}
	}
}
func mvDownloader(adamID string, saveDir string, token string, storefront string, mediaUserToken string, meta *structs.AutoGenerated) error {
	...
	if mvm3u8url == "" {
		return errors.New("media-user-token可能无效或过期")
	}
	...
	if true {
		...
		if err != nil {
			fmt.Println("MV缩略图保存失败:", err)
		} else {
			//fmt.Println("MV缩略图保存成功")
			tags = append(tags, fmt.Sprintf("cover=%s", covPath))
		}
	}
...
func extractMvAudio(c string) (string, error) {
	...
	fmt.Println("音频流: " + audioStreams[0].GroupID)
...
func checkM3u8(b string, f string) (string, error) {
	...
	if Config.GetM3u8FromDevice {
		...
		if err != nil {
			fmt.Println("向解密服务写长度时出错:", err)
			return "none", err
		}

		_, err = conn.Write(adamIDBuffer)
		if err != nil {
			fmt.Println("向解密服务写入adamID时出错:", err)
			return "none", err
		}
		...
		if len(response) > 0 {
			if f == "song" {
				fmt.Println("接收到的URL:", string(response))
			}
			EnhancedHls = string(response)
		} else {
			fmt.Println("接收的URL为空.")
		}
	}
...
func extractMedia(b string, more_mode bool) (string, string, error) {
	...
	if debug_mode && more_mode {
		...
		table := tablewriter.NewWriter(os.Stdout)
		table.SetHeader([]string{"编码", "音频", "带宽"})
		//table.SetFooter([]string{"", "", "Total", "$146.93"})
		table.SetAutoMergeCells(true)
		//table.SetAutoMergeCellsByColumnIndex([]int{1,2,3})
			...
			if variant.Codecs == "mp4a.40.2" { // AAC
				...
				if len(split) >= 3 {
					...
					if bitrate > currentBitrate {
						aacQuality = fmt.Sprintf("AAC | 2 声道 | %d kbps", bitrate)
					}
				}
				...
				if len(split) > 0 {
					...
					if bitrate > currentBitrate {
						atmosQuality = fmt.Sprintf("E-AC-3 | 16 声道 | %d kbps", bitrate)
					}
				}
				...
				if len(split) >= 3 {
					...
					if sampleRateInt > 48000 { // Hi-Res
						hasHiRes = true
						hiResQuality = fmt.Sprintf("ALAC | 2 声道 | %s-bit/%d kHz", bitDepth, sampleRateInt/1000)
					} else { // Standard Lossless
						hasLossless = true
						losslessQuality = fmt.Sprintf("ALAC | 2 声道 | %s-bit/%d kHz", bitDepth, sampleRateInt/1000)
					}
				}
				...
				if len(split) > 0 {
					...
					dolbyAudioQuality = fmt.Sprintf("AC-3 |  16 声道 | %d kbps", bitrate)
				}
			}
		}
	...
	for _, variant := range master.Variants {
		if dl_atmos {
			if variant.Codecs == "ec-3" && strings.Contains(variant.Audio, "atmos") {
				if debug_mode && !more_mode {
					fmt.Printf("发现杜比音频-%s(Bitrate: %d kbps)\n",
				...
				if debug_mode && !more_mode {
					fmt.Printf("发现杜比全景声的变种 - %s (Bitrate: %d kbps)\n",
						variant.Audio, variant.Bandwidth/1000)
				}
				streamUrlTemp, err := masterUrl.Parse(variant.URI)
				if err != nil {
			...
			if variant.Codecs == "mp4a.40.2" {
				if debug_mode && !more_mode {
					fmt.Printf("发现杜比全景声的变种 - %s (Bitrate: %d)\n", variant.Audio, variant.Bandwidth)
				}
	...
	for _, variant := range video.Variants {
		...
		if len(matches) == 3 {
			...
			if h <= maxHeight {
				...
				fmt.Println("视频流: " + variant.Resolution + "-" + variant.VideoRange)
				break
			}
		}
	}
